...
public class UnifiedDiffParser implements DiffParser {
    public static final Pattern LINE_RANGE_PATTERN = ~/^.*-([0-9]+)(?:,([0-9]+))? \+([0-9]+)(?:,([0-9]+))?.*$/

    @Override
    public List<Diff> parse(InputStream inputStream) {
        ResizingParseWindow window = new ResizingParseWindow(inputStream)
